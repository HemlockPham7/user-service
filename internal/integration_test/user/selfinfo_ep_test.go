...
func TestUserEndpoint_GetSelfInfo(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			setupTestHTTP: func(api api.Engine) *httptest.ResponseRecorder {
				req, rec := setupRequestUserSelfInfo(true)
			...
			setupRateLimit: func(ctx context.Context, redisClient *redis.Client) *redis.Client {
				return setupRateLimitUserSelfInfo(ctx, 1, redisClient)
			},

			expectedStatusCode:   http.StatusOK,
			expectedResponseBody: `"id":"d7c13097-67a7-4eae-a60e-0b9b533b7bd4","created_at":"2023-01-01T00:00:00Z","updated_at":"2023-01-01T00:00:00Z","display_name":"John Doe","username":"johndoe","email":"johndoe@gmail.com"`,
		},
	}
...
func setupRequestUserSelfInfo(haveClaims bool) (*http.Request, *httptest.ResponseRecorder) {
...
func setupRateLimitUserSelfInfo(ctx context.Context, rateLimit int, redisClient *redis.Client) *redis.Client {
